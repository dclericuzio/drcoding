import {data_drcoding} from '../../models/index'
...
export default async function handler(req, res) {
    switch(req.method){
         ...
         try {
             ...
             const data = await data_drcoding.create({name: name, email:email, phone_number: phoneNumber, types:project, message:message})
             if(!data){
         ...
         case "GET":
             ...
             try {
                 ...
                 const data = await data_drcoding.findAll({order: [
                     ["createdAt", "desc"]
                 ]})
                 if(data.length<1){
